...
class Controller:
    def __init__(self, robot_name: str, viewer = True):
        ...
        self.torque_index = [0] * self.model.nu
    ...
    def _start(self):
        ...
        if not self._running.is_set():
            ...
            # Find de model.sensordata index of the actuators' joints
            for actuator in range(self.model.nu):
                joint = self.model.actuator_trnid[actuator, 0]
                axes = self.model.jnt_axis[joint]
                index = next((i for i in range(3) if axes[i] == 1), None)
                if index is None:
                    raise Exception('Critical error: Actuator without an associated joint.')
                self.torque_index[actuator] = (joint*3)+index

            # Starts the threads
            self._running.set() # lets work
    ...
    def get_torque(self, servo: int) -> float:
        ...
        with self._lock:
            ...
            return self.data.sensordata[self.torque_index[servo]]
...
class Servo:
    def __init__(self, controller, unit_converter, servo_id, pos_limits=[-30,30], vel_limits=[5,50], tor_limits=[-1,1], position=0, velocity=30, reverse=False):
        '''
        ...
        tor_limits (tuple/list): Torque limits (min, max) in percentage.
    ...
    def get_torque(self) -> float: # or force
        ...
        '''
        tor_phys =  self.controller.get_torque(self.servo_id)
        return tor_phys #self.UC.torque.user_units(tor_phys)
